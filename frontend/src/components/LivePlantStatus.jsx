    ...
    <>
      ...
      <div className="plant-grid">
        {(plantStatus || []).map((item, index) => (
          ...
          >
            <h3>{item.device || "Unknown Rail Asset"}</h3>
            ...
            {item.temperature !== undefined && (
              <p>
                <strong>Equipment Temperature:</strong> {item.temperature}°F
              </p>
            )}

            {item.power_output_kw !== undefined && (
              <p>
                <strong>Backup Power:</strong> {item.power_output_kw} kW
              </p>
            )}

            {item.voltage !== undefined && (
              <p>
                <strong>Supply Voltage:</strong> {item.voltage} V
              </p>
            )}

            {item.cpu_usage !== undefined && (
              <p>
                <strong>Controller CPU:</strong> {item.cpu_usage}%
              </p>
            )}

            {item.memory_usage !== undefined && (
              <p>
                <strong>Controller Memory:</strong> {item.memory_usage}%
              </p>
            )}

            {item.active_sessions !== undefined && (
              <p>
                <strong>Remote Sessions:</strong> {item.active_sessions}
              </p>
            )}

            {item.failed_logins !== undefined && (
              <p>
                <strong>Failed Authentication:</strong> {item.failed_logins}
              </p>
            )}
